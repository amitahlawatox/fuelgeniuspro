    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="text-sm text-gray-400 mb-6 flex items-center gap-2">
          ...
          <span>/</span>
          <Link href="/fuel-tracker" className="hover:text-gray-500">Fuel Tracker</Link>
          <span>/</span>
        ...
        <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-5 mb-8 flex flex-col sm:flex-row gap-4 sm:items-center">
          ...
          <div>
            <p className="font-bold text-gray-900 text-lg">Cheapest petrol in {city.name}: <span className="text-green-400">{cheapest.petrol?.toFixed(1)}p/litre</span></p>
        ...
        {/* Station table */}
        <h2 className="text-xl font-bold text-gray-900 mb-4">All Stations in {city.name}</h2>
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-8">
          <table className="w-full text-sm">
            <thead className="bg-gray-100 text-gray-400 text-xs uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-gray-100">
              {stations.map((s, i) => {
                ...
                return (
                  <tr key={s.id} className={`hover:bg-gray-100/50 transition-colors ${i === 0 ? 'bg-green-500/5' : ''}`}>
                    ...
                    <td className={`px-4 py-3 text-right text-xs hidden sm:table-cell ${parseFloat(diff) < 0 ? 'text-green-400' : parseFloat(diff) > 0 ? 'text-red-400' : 'text-gray-400'}`}>
        ...
        {city.nearby.length > 0 && (
          <div>
            <h2 className="text-lg font-bold text-gray-900 mb-3">Nearby Areas</h2>
            <div className="flex flex-wrap gap-2">
              {city.nearby.map(slug => (
                <Link key={slug} href={`/fuel/${slug}`} className="bg-white border border-gray-200 px-3 py-1.5 rounded-lg text-sm text-gray-500 hover:text-green-400 hover:border-green-500/30 transition-colors capitalize">
                  {slug.replace(/-/g, ' ')}
                </Link>
              ))}
            </div>
        ...
        <p className="mt-8 text-xs text-gray-500">Fuel price data sourced under the UK Open Government Licence v3.0. © Competition and Markets Authority. Prices are for guidance only and may vary.</p>
      </div>
    </>
  )
}
