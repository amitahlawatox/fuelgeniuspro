  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex items-center gap-3 mb-2">
        ...
        <h1 className="text-3xl font-bold text-white">Journey Cost Calculator</h1>
      </div>
      <p className="text-gray-400 mb-8">Enter your journey distance and car to calculate exact fuel cost.</p>

      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">Journey distance (miles)</label>
          <input type="number" min="1" value={miles} onChange={e=>setMiles(e.target.value)}
            placeholder="e.g. 150"
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500" />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">Your car</label>
        ...
        {isCustom && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Your MPG</label>
        ...
        <div>
          ...
          <input type="number" step="0.1" value={pricePer} onChange={e=>setPricePer(e.target.value)}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-green-500" />
          <p className="text-xs text-gray-500 mt-1">UK average today: {fuelType==='petrol'?UK_AVG_PRICES.petrol:UK_AVG_PRICES.diesel}p/litre</p>
        </div>

        <button onClick={calculate}
          className="w-full bg-green-600 hover:bg-green-500 text-white font-semibold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2">
      ...
      {result && (
        <div className="mt-6 bg-green-500/10 border border-green-500/20 rounded-xl p-6">
          <h2 className="font-bold text-white text-xl mb-4">Your Journey Cost</h2>
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <p className="text-3xl font-extrabold text-white">{result.litres.toFixed(1)}L</p>
              <p className="text-sm text-gray-400 mt-1">Fuel used</p>
            </div>
            <div>
              <p className="text-3xl font-extrabold text-white">{(result.cost / parseFloat(miles) * 100).toFixed(1)}p</p>
